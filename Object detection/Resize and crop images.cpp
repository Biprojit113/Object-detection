...
/*
	...
	Mat image = imread(path);                       // for reading image from this path
	
	Mat imgResize, imgCrop;                               // declaring the variables

	//cout << image.size() << endl;                 //for printing the size of my image

	resize(image, imgResize, Size(), 0.5, 0.5);       // using resize function 

	Rect img(100, 240, 270, 400);                // using a rectangle for maping the crop size x,y,width,height

	imgCrop = image(img);



	imshow("Alone", image);					// for displaying the image

	imshow("Alone Resize", imgResize);

	imshow("Alone Crop", imgCrop);
	...
	*/
	...
	putText(image, "What's UP Bro!!", Point(80, 210), FONT_HERSHEY_SCRIPT_COMPLEX, 1, Scalar(0, 0, 0), 1);*/
